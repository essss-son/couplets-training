...
if __name__ == '__main__':
    ...
    trans = MyTransformer(d_model, num_head, num_layers, num_layers, dim_feedforward=128)
    attn_mask = trans.generate_square_subsequent_mask(tgt_len)
    result = trans(src, tgt, tgt_mask=attn_mask,
    ...
    result = tra(src, tgt, tgt_mask=attn_mask,
                 src_key_padding_mask=src_key_padding_mask,
                 tgt_key_padding_mask=tgt_key_padding_mask,
                 memory_key_padding_mask=src_key_padding_mask)
    print(result.shape)
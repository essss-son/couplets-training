...
def greedy_decode(model, src, max_len, start_symbol, config, data_loader):
    src = src.to(config.device)  # 本例中src的形状是[src_len,1]
    ...
    for i in range(max_len - 1):  # 逐时刻进行解码
        ...
        prob = model.classifier(out[:, -1, :])  # 只对预测的下一个词进行分类，即最后一个时刻
...
def translate(model, src, data_loader, config):
    ...
    src = (torch.LongTensor(tokens).reshape(num_tokens, 1))  # 将src_len 作为第一个维度  [src_len, batch_size]
    with torch.no_grad():
...
def translate_german_to_english(srcs, config):
    ...
    translation_model = Translation_model(src_vocab_size=len(data_loader.de_vocab),
    ...
    loaded_paras = torch.load(config.model_save_path + '/model.pt')
